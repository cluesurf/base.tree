...
class GraphemerIterator {
    ...
    next() {
        let brk;
        if ((brk = Graphemer.nextBreak(this._str, this._index)) < this._str.length) {
...
function shouldBreak(start, mid, end, startEmoji, midEmoji, endEmoji) {
    ...
    // GB4. (Control|CR|LF) ÷
    else if (previous === CLUSTER_BREAK.CONTROL ||
        previous === CLUSTER_BREAK.CR ||
        previous === CLUSTER_BREAK.LF) {
        return BreakStart;
    }
    // GB5. ÷ (Control|CR|LF)
    else if (next === CLUSTER_BREAK.CONTROL ||
        next === CLUSTER_BREAK.CR ||
        next === CLUSTER_BREAK.LF) {
        return BreakStart;
    }
    // GB6. L × (L|V|LV|LVT)
    else if (previous === CLUSTER_BREAK.L &&
    ...
    // GB7. (LV|V) × (V|T)
    else if ((previous === CLUSTER_BREAK.LV || previous === CLUSTER_BREAK.V) &&
        (next === CLUSTER_BREAK.V || next === CLUSTER_BREAK.T)) {
        return NotBreak;
    }
    // GB8. (LVT|T) × (T)
    else if ((previous === CLUSTER_BREAK.LVT || previous === CLUSTER_BREAK.T) &&
    ...
    if (previous === CLUSTER_BREAK.REGIONAL_INDICATOR &&
...
function getNextBreak(string, index) {
    ...
    for (let i = index + 1; i < string.length; i++) {
        ...
        if (GraphemerHelper.shouldBreak(prev, mid, next, prevEmoji, midEmoji, nextEmoji)) {
            return i;
        }
        mid.push(next);
